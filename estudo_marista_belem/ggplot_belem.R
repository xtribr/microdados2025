#!/usr/bin/env Rscript
# Suíte por escola — Marista Belém (aplicação COP30/BAM). Igual ao pipeline por
# escola, com a referência item a item trocada: Coorte COP30 (Belém-Ananindeua-
# Marituba, ~62-66 mil presentes, MESMA prova) no lugar do Brasil Regular P1.
...
SCOL <- c("#8C9298"); names(SCOL) <- REF
SCOL <- c(setNames(COR, ESC), SCOL)
NOMES <- c(LC = "Linguagens", CH = "Ciências Humanas", CN = "Ciências da Natureza", MT = "Matemática")
NITENS <- c(LC = 45, CH = 45, CN = 45, MT = 45)  # prova BAM: zero itens anulados
res <- fromJSON(file.path(BASE, "resumo.json"))
CAP <- sprintf("Estudo XTRI — Prof. Alexandre Emerson  |  Microdados ENEM 2025 / INEP  |  Marista Nazaré (Belém/PA): %d alunos na prova COP30/BAM",
               res$processados_bam)
...
for (a in names(NOMES)) {
  ...
  dn <- d[!is.na(d$erro_cop30) & d$erro_cop30 != "", ]
  dn$erro_cop30 <- as.numeric(dn$erro_cop30)
  p <- ggplot(d, aes(x = erro_pct, y = habilidade)) +
    geom_col(fill = COR, width = 0.7) +
    geom_point(data = dn, aes(x = erro_cop30, y = habilidade),
    ...
    labs(title = sprintf("%s — onde o %s mais erra", NOMES[a], ESC),
         subtitle = "Barra = escola · losango preto = coorte COP30 (mesma prova). Barra passando do losango = erra mais que a coorte.",
...
for (a in names(NOMES)) {
  ...
  b <- data.frame(escola = REF, categoria = da$categoria, acerto = da$acerto_cop30)
  d <- rbind(e, b)
  d$categoria <- factor(d$categoria, levels = CATS)
  d$escola <- factor(d$escola, levels = names(SCOL))
  d$xn <- as.numeric(d$categoria)
  r_fac <- d$acerto[d$xn == 1 & d$escola == ESC] / d$acerto[d$xn == 1 & d$escola == REF]
  r_md  <- d$acerto[d$xn == 4 & d$escola == ESC] / d$acerto[d$xn == 4 & d$escola == REF]
  br_md <- d$acerto[d$xn == 4 & d$escola == REF]
  es_md <- d$acerto[d$xn == 4 & d$escola == ESC]
  br_fac <- d$acerto[d$xn == 1 & d$escola == REF]
  p <- ggplot(d, aes(x = xn, y = acerto, colour = escola, group = escola)) +
    ...
    labs(title = sprintf("%s — o degrau da dificuldade", NOMES[a]),
         subtitle = "Como ler: toda linha cai — item difícil é difícil pra todos. Categorias = quartis de dificuldade dos 45 itens da prova COP30/BAM.",
...
b <- data.frame(escola = REF, area = dsc$area, cat = dsc$faixa_disc, acerto = dsc$acerto_cop30)
d2 <- rbind(e, b)
...
p3 <- ggplot(d2, aes(x = cat, y = acerto, colour = escola, group = escola)) +
  ...
  geom_text(data = d2[d2$escola == REF, ], aes(label = sprintf("%.0f", acerto)),
  ...
  labs(title = "Item que separa quem sabe: a distância cresce com o poder do item",
       subtitle = "Quanto mais discriminativo o item (parâmetro A), mais a linha da coorte COP30 (tracejado) se afasta da escola.",
...
for (a in names(NOMES)) {
  ...
  p <- ggplot() +
    ...
    labs(title = sprintf("Gráfico de incoerência — %s (%s)", NOMES[a], ESC),
         subtitle = "Cinza = coorte COP30 (mesma prova). Ponto colorido = aluno; tamanho = incoerência (erros em fáceis × acertos em difíceis).",
         x = sprintf("Acertos em %s (de %d itens)", NOMES[a], NITENS[a]),
...
# ---------- 5) NUVEM DA COORTE (4 áreas juntas) ----------
alp <- read.csv(file.path(BASE, "alunos_proc.csv"), stringsAsFactors = FALSE, encoding = "UTF-8")
alp <- alp[alp$areas == 4, ]
nac <- read.csv(file.path(BASE, "chutes_scatter_cop30.csv"), stringsAsFactors = FALSE)
p5 <- ggplot() +
  ...
  labs(title = sprintf("Onde os alunos do %s caem na nuvem da COP30", ESC),
       subtitle = "Cinza = todos os presentes-4 da aplicação COP30/BAM (Belém-Ananindeua-Marituba). Ponto colorido = aluno; tamanho = índice de incoerência.",
       x = "Acertos nas 4 provas objetivas (de 180 itens)", y = "Nota TRI média (4 áreas)", caption = CAP) +
  theme_xtri(gridx = TRUE)
ggsave(file.path(OUT, "chutes.png"), p5, width = 13.33, height = 7.5, dpi = 150, bg = "#F1F1F2")
cat("ok: chutes\n")

# ---------- 6) REDAÇÃO ----------
# Comparação com a MESMA aplicação (tema de redação da COP30/BAM difere do regular).
CLAB <- c("C1\nnorma culta", "C2\ncompreender\no tema", "C3\nargumentação", "C4\ncoesão", "C5\nproposta de\nintervenção")
rc <- fromJSON(file.path(BASE, "redacao_comp.json"))[[ESC]]
refc <- fromJSON(file.path(BASE, "referencia_cop30.json"))$redacao_comp
rows <- list()
for (i in 1:5) {
  rows[[length(rows) + 1]] <- data.frame(escola = ESC, comp = i, media = rc[[paste0("C", i)]]$media,
                                         p200 = rc[[paste0("C", i)]]$pct_200)
  rows[[length(rows) + 1]] <- data.frame(escola = REF, comp = i,
                                         media = refc[[paste0("C", i)]]$media, p200 = NA)
}
...
p6 <- ggplot(d6, aes(x = comp, y = media, colour = escola, group = escola)) +
  ...
  geom_text(data = d6[d6$escola == REF, ], aes(label = vir(media, 0)), vjust = 2.4,
            size = 4.2, show.legend = FALSE) +
  annotate("segment", x = gg - 0.22, xend = gg - 0.22, y = gg_m + 4, yend = 199,
           colour = CORd, linewidth = 1.1,
           arrow = arrow(ends = "both", length = unit(0.16, "cm"), type = "closed")) +
  annotate("label", x = 0.9, y = 60,
  ...
  scale_y_continuous(limits = c(40, 215), breaks = seq(40, 200, 40)) +
  labs(title = sprintf("Redação — onde o %s ainda perde ponto", ESC),
       subtitle = "Média por competência (0–200), escola × coorte COP30 (tracejado) — mesma aplicação, mesmo tema de redação.",
